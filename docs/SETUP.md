...
> - 有効期限があります。切れると週次リサーチが失敗するようになります。
>   復旧手順は「[7. 困ったときは](#7-困ったときは)」にまとめてあります。
...
- [x] `src/config.ts` のサイト名・運営者名を設定した（連絡先は未設定）
...
### 週次リサーチが失敗する — まずトークンの期限を疑う

**これまで動いていたのに、ある週から急に失敗し始めた場合、原因はほぼトークンの期限切れです。**

`CLAUDE_CODE_OAUTH_TOKEN` には有効期限があり、切れると認証が通らなくなります。
GitHub からは何の予告もありません。

**症状**

Actions → 「週次リサーチと公開」の失敗した実行を開き、`research` ジョブの
**「リサーチと記事生成」** ステップを展開します。次のような表示があれば期限切れです。
...
失敗が数十秒で終わっているのも目印です（記事生成に入る前に止まるため）。
...
1. 手元のターミナルで発行し直す
   ...
   ブラウザが開いて認証を求められます。完了するとトークンが表示されるのでコピーします。

2. GitHub の Secrets を更新する

   リポジトリの **Settings → Secrets and variables → Actions** を開き、
   `CLAUDE_CODE_OAUTH_TOKEN` の行にある**鉛筆アイコン**をクリックします。
   新しい値を貼り付けて **Update secret**。

   > 削除して作り直す必要はありません。名前が変わると
   > ワークフローから参照できなくなるので、必ず既存の項目を編集してください。

3. 動作を確認する

   Actions → 「週次リサーチと公開」→ **Run workflow**、
   `dry_run` に**チェックを入れて**実行します。
   成功すれば復旧完了です。記事は書き込まれないので、確認用に何度でも実行できます。

**予防**

期限切れは避けられませんが、動かなくなってから気づくまでの空白は短くできます。
週次実行が失敗すると GitHub から通知メールが届くので、それを見逃さないようにしてください
（届かない場合は GitHub の Settings → Notifications → Actions を確認）。

気になるなら、半年に一度くらい先回りして発行し直しておくのも手です。
古いトークンが残っていても、Secrets を新しい値で上書きすれば新しいほうが使われます。

### そのほかの失敗

| 症状 | 原因と対処 |
| --- | --- |
| `Could not fetch an OIDC token` | ジョブに `id-token: write` の権限がない。ワークフローの `permissions` を確認する（設定済み） |
| `Invalid username or token`（push時） | `claude-code-action` が git の認証情報を失効させた後に push している。`git remote set-url` で張り直す（設定済み） |
| Cloudflare で「有効な URL がありません」 | `wrangler.toml` の `workers_dev = true` が無効。または対象Worker → 設定 → ドメインとルートから有効化する（設定済み） |
| GitHub Pages のURLが表示されない | まだ一度もデプロイが成功していない。Settings → Pages の Source が「GitHub Actions」か確認する |
| 検証スクリプトで失敗する | エージェントが手順書のルールを守れていない。ログに違反内容が出るので、`docs/agent/weekly-research.md` の指示を明確にして再実行する |
| 手元で Claude Code がサブスクを使ってくれない | `ANTHROPIC_API_KEY` が環境変数に設定されていると、そちらが優先されて従量課金になる。`echo $ANTHROPIC_API_KEY` で確認する |
